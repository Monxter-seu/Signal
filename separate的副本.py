...
from process import ProcessingLayer, KNNClassifier
...
def separate(args):
    ...
    # Load model
    conv = ConvTasNet.load_model(args.model_path)
    #取出最后一层并添加新的层
    model = nn.Sequential(conv, ProcessingLayer, KNNClassifier)
    ...
    with torch.no_grad():
        for (i, data) in enumerate(eval_loader):
            ...
            # Forward
            estimate_source = model(mixture)  # [B, C, T]的结果需要去除paddiing并降维,这部分内容需要放入网络中
