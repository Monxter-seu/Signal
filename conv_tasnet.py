...
class ConvTasNet(nn.Module):
    ...
    def forward(self, mixture):
        ...
        '''
        for i in range(0,2):
            mixture_w_array = mixture_w[i].cpu().numpy()
            io.savemat('D:/Conv-TasNet-master/mixture_w_mat/'+filename[i][16:]+'.mat',{'mixture_w_array':mixture_w_array})
            #numpy.savetxt('D:/Conv-TasNet-master/mixture_w/'+filename[i][20:]+'.txt',mixture_w_array)#保存为txt
        '''
        est_mask = self.separator(mixture_w)
        #print('est_mask.shape',est_mask.shape)
        #print('est_mask',est_mask)
        #print('est_mask[0][0]',est_mask[0][0])
        #输出est_mask
        '''
        for i in range(0,2):
                for j in range(0,2):
                    est_mask_array = est_mask[i][j].cpu().numpy()
                    io.savemat('D:/Conv-TasNet-master/est_mask_mat/'+filename[i][16:]+'_s'+str(j+1)+'.mat',{'est_mask_array':est_mask_array})
            #numpy.savetxt('D:/Conv-TasNet-master/mixture_w/'+filename[i][20:]+'.txt',mixture_w_array)#保存为txt       
        '''
        ...
        print('est_source.shape',est_source.shape)
        return est_source
...
class Encoder(nn.Module):
    ...
    def forward(self, mixture):
        ...
        #print('self.conv1d_U(mixture)',self.conv1d_U(mixture).shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, mixture_w, est_mask):
        ...
        for i in range(0,2):
                for j in range(0,2):
                    source_w_array = source_w[i][j].cpu().numpy()
                    io.savemat('D:/Conv-TasNet-master/source_w_mat/'+filename[i][16:]+'_s'+str(j+1)+'.mat',{'source_w_array':source_w_array})
        '''
